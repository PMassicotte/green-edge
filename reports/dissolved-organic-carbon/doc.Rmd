...
knitr::opts_chunk$set(echo = FALSE, warning = FALSE, fig.width = 7, fig.align = "center")
...
raimbault <- readxl::read_excel("~/Desktop/doc.xlsx", sheet = 1) %>% 
  janitor::clean_names() %>% 
  select(
    jday = station,
    depth,
...
christos <- readxl::read_excel("~/Desktop/doc.xlsx", sheet = 2) %>% 
  janitor::clean_names() %>% 
  select(
    jday = station,
    depth,
...
atsushi <- readxl::read_excel("~/Desktop/doc.xlsx", sheet = 3) %>% 
  janitor::clean_names() %>% 
  select(
    jday = julian_day,
    depth = depth_m,
